...
public class FeatureMap implements Serializable {
   ...
   public int getIndex(int featurePosition, int code) {
      Integer index = (Integer)this.map.get((long)featurePosition << 48 | (long)code);
   ...
   public int setIndex(long key, int index) {
      return (Integer)this.map.put(key, index);
   }
   ...
   public int getIndex(int featurePosition, int code1, int code2) {
      Integer index = (Integer)this.map.get((long)featurePosition << 48 | (long)code1 << 24 | (long)code2);
      return index == null ? -1 : index;
   }
   ...
   public Long[] reverseMap() {
      ...
      Long key;
      for(Iterator i$ = this.map.keySet().iterator(); i$.hasNext(); reverseMap[(Integer)this.map.get(key)] = key) {
         key = (Long)i$.next();
      }
