...
public class FeaturesExtraction {
    ...
    public Item word2feature(String sequence, int i) {
        ...
        if (i > 0) {
            String word_pre = words[i - 1];
            String word_pre_word = "w[-1]=" + word_pre;
            item.add(new Attribute(word_pre_word));
        }
        if (i > 1) {
            String word_pre2 = words[i - 2];
            String word_pre2_word = "w[-2]=" + word_pre2;
            item.add(new Attribute(word_pre2_word));
        }
        if (i < words.length - 1) {
            String word_post = words[i + 1];
            String word_post_word = "w[1]=" + word_post;
            item.add(new Attribute(word_post_word));
        }
        if (i < words.length - 2) {
            String word_post2 = words[i + 2];
            String word_post2_word = "w[2]=" + word_post2;
